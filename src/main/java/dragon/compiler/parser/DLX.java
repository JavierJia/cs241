...
public class DLX {
	...
	static int REG_RETURN_PC = 31;
	static int REG_GLOBAL = 30;
	static int REG_STACK = 29;
	static int REG_FRAME = 28;
	static int REG_RETURN_VALUE = 27;
	...
	public static void execute(boolean debug) throws IOException {
		...
		try {
			...
			execloop: while (true) {
				...
				case CHK:
					if (R[a] < 0) {
						System.out.println("DLX.execute: " + PC * 4 + ": R[" + a + "] == " + R[a]
								+ " < 0");
						bug(14);
					} else if (R[a] >= c) {
						System.out.println("DLX.execute: " + PC * 4 + ": R[" + a + "] == " + R[a]
...
//					System.out.println(PC + " PSH: MEM[" + R[b] + "] = R[" + a + "]" + " = " + R[a]);
				...
				case RET:
					if (origc == 0)
						break execloop; // remember: c==R[origc]
					if ((c < 0) || (c > MemSize)) {
	...
	static final String mnemo[] = { "ADD", "SUB", "MUL", "DIV", "MOD", "CMP", "ERR", "ERR", "OR",
			"AND", "BIC", "XOR", "LSH", "ASH", "CHK", "ERR", "ADDI", "SUBI", "MULI", "DIVI",
			"MODI", "CMPI", "ERRI", "ERRI", "ORI", "ANDI", "BICI", "XORI", "LSHI", "ASHI", "CHKI",
			"ERR", "LDW", "LDX", "POP", "ERR", "STW", "STX", "PSH", "ERR", "BEQ", "BNE", "BLT",
			"BGE", "BLE", "BGT", "BSR", "ERR", "JSR", "RET", "RDI", "WRD", "WRH", "WRL", "ERR",
	...
	static int map2constOp(int op) {
		if (op < 0 || op > 14) {
			throw new IllegalArgumentException("op code is not valide");
		}
		return op + 16;
	}
	...
	static int[] getJSRDetail(int instructionWord) {
		return new int[] { instructionWord >>> 26, instructionWord & 0x3FFFFFF };
	}
